...
(see :meth:`BinaryValueCriticModel._predicted_signed_value`), rather than
...
# The recap-branch data/model modules are absent on the steam branch. Only
# ``expected_signed_stride`` is used by the render/plot helpers; the dataset
# collator and critic classes are needed solely by this module's own
# model-loading entry point (not exercised by visualize_advantage_ensemble.py).
try:  # noqa: E402
...
except ModuleNotFoundError:  # steam branch fallback
    from rlinf.data.datasets.steam.binning import expected_signed_stride  # noqa: E402

    BinaryPairDataCollator = None  # type: ignore[assignment]
    PairDataset = None  # type: ignore[assignment]
try:  # noqa: E402
    from rlinf.models.embodiment.value_model_rewind_arm.modeling_critic import (  # noqa: E402
        BinaryValueCriticModel,
    )
except ModuleNotFoundError:  # steam branch fallback
    BinaryValueCriticModel = None  # type: ignore[assignment]
...
def detect_image_keys(sample: dict) -> list[str]:
    ...
    for key in sample.keys():
        if (
            ...
            or key.endswith(("_view", "_image", "_rgb"))
            or key in (
                "image",
                "wrist_image",
                "front_image",
        ...
        if is_multi_bin:
            ...
            if result.member_probs is None:
                raise RuntimeError(
                    ...
                    "BinaryValueCriticModel.from_checkpoint which routes "
...
def _create_episode_summary_plot(
    ...
    :meth:`BinaryValueCriticModel._predicted_signed_value`).
    ...
    # STEAM: append dataset / final-success / takeover, injected per-episode by
    # the caller (render_per_episode) as ``ep_data['title_extra']``.
    suptitle_head += ep_data.get("title_extra", "")
    suptitle_body = (
