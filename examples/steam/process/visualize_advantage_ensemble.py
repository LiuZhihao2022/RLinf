...
def _load_episode_success_takeover(dataset_path: Path) -> dict:
    """Read ``meta/episodes.jsonl`` → ``{episode_index: (is_success, takeover)}``.

    Both fields are optional (``None`` when absent). Used to annotate each
    per-episode plot title with final success and whether a human took over.
    """
    import json as _json

    meta: dict = {}
    ep_file = Path(dataset_path) / "meta" / "episodes.jsonl"
    ...
    with open(ep_file) as f:
        for line in f:
            ...
            if idx is None:
                continue
            meta[int(idx)] = (d.get("is_success"), d.get("takeover"))
    return meta
...
def render_per_episode(
    ...
    ep_meta = _load_episode_success_takeover(dataset_path)
    ...
    for ep in _tqdm(selected, desc="rendering episodes"):
        ...
        # STEAM title annotation: dataset name / final success / human takeover.
        _succ, _take = ep_meta.get(int(ep), (None, None))
        _bits = [f"[{dataset_name}]"]
        if _succ is not None:
            _bits.append(f"final={'SUCCESS' if _succ else 'FAIL'}")
        if _take is not None:
            _bits.append(f"takeover={'YES' if _take else 'NO'}")
        ep_data["title_extra"] = "\n" + "  ".join(_bits)
