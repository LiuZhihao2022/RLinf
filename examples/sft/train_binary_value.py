...
"""Entry point for ARM + ReWiND Binary Value Model SFT training.

Usage:
    python train_binary_value.py --config-path config --config-name rewind_arm_value_model
    python train_binary_value.py --config-path config --config-name rewind_arm_value_test
"""
...
from rlinf.workers.value_sft.fsdp_binary_value_sft_worker import (  # noqa: E402
    FSDPBinaryValueSftWorker,
)
...
@hydra.main(
    version_base="1.1",
    config_path="config",
    config_name="rewind_arm_value_model",
)
def main(cfg) -> None:
    ...
    actor_group = FSDPBinaryValueSftWorker.create_group(cfg).launch(
