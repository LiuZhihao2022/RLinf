...
"""Entry point for frame-level Success/Fail Classifier SFT training.

Usage:
    python train_success_fail_classifier.py \
        --config-path config --config-name libero_sft_success_fail_classifier
"""
...
from rlinf.workers.value_sft.fsdp_frame_classifier_sft_worker import (  # noqa: E402
    FSDPFrameClassifierSftWorker,
)
...
def main(cfg) -> None:
    ...
    actor_group = FSDPFrameClassifierSftWorker.create_group(cfg).launch(
        cluster,
