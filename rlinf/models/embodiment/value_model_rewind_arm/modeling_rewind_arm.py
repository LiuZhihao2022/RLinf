...
"""Backbone for the ARM + ReWiND binary pair classifier.
...
The stack — SigLIP vision encoder + Gemma3 language backbone + an MLP
head — parallels the evorl Pistar06 backbone that lives next to us in
``value_model_evorl``, but the forward path is designed *for pair
classification*, not for a scalar value regression, and the two modules
intentionally do not share code:
...
``RewindArmBackbone.forward`` returns the raw logits ``[B, 2]``; the
...
class RewindArmBackbone(nn.Module):
    """SigLIP + Gemma3 backbone for ARM+ReWiND pair classification.

    The forward path takes an observation dict whose ``images`` slot carries
    exactly :attr:`BinaryValueConfig.num_frames_per_pair` camera entries —
    ...
    def __init__(self, cfg: "BinaryValueConfig") -> None:
        super().__init__()
