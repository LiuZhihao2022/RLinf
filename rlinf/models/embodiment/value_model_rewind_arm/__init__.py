...
_BINARY_VALUE_CONFIG_DEFAULTS: dict[str, Any] = {
    "vision_repo_id": "",
    "language_repo_id": "",
...
assert all(
    ...
    for v in _BINARY_VALUE_CONFIG_DEFAULTS.values()
), (
    "_BINARY_VALUE_CONFIG_DEFAULTS must stay flat: build_binary_value_config "
...
def load_binary_value_checkpoint_config(
    model_path: str | os.PathLike[str] | None,
) -> BinaryValueConfig | None:
    """Load ``BinaryValueConfig`` saved alongside a checkpoint if available."""
    for checkpoint_dir in _candidate_checkpoint_dirs(model_path):
        config_path = os.path.join(checkpoint_dir, "config.json")
        if not os.path.exists(config_path):
            continue
        try:
            config = BinaryValueConfig.from_pretrained(
                checkpoint_dir,
                local_files_only=True,
            )
        except Exception as exc:  # pragma: no cover - defensive logging path
            logger.warning(
                "Failed to load BinaryValueConfig from %s: %s",
                checkpoint_dir,
                exc,
            )
            continue
        logger.info(
            "Loaded BinaryValueConfig metadata from checkpoint directory %s",
            checkpoint_dir,
        )
        return config
    return None
...
def _resolve_precision_dtype(precision) -> str:
    ...
    the value back from a saved ``BinaryValueConfig.dtype`` in
...
def build_binary_value_config(
    cfg: DictConfig,
    checkpoint_config: BinaryValueConfig | None = None,
) -> BinaryValueConfig:
    """Build config from live Hydra values plus checkpoint metadata fallback.

    Precedence: ``_BINARY_VALUE_CONFIG_DEFAULTS`` ← ``checkpoint_config`` ← ``cfg``.
    ...
    if checkpoint_config is None:
        checkpoint_config = load_binary_value_checkpoint_config(
            getattr(cfg, "model_path", None)
        )

    merged = dict[str, Any](_BINARY_VALUE_CONFIG_DEFAULTS)
    for source_label, source in (
        ...
        if source is None:
            continue
        for field in _BINARY_VALUE_CONFIG_DEFAULTS:
            ...
            if isinstance(value, (dict, DictConfig)):
                raise TypeError(
                    ...
                    f"build_binary_value_config merges non-recursively and "
    ...
    return BinaryValueConfig(**merged)


def save_binary_value_checkpoint_assets(
    save_path: str,
    cfg: DictConfig | BinaryValueConfig,
    processor: Any | None = None,
) -> None:
    """Persist config/tokenizer/image-processor assets next to checkpoint weights."""
    os.makedirs(save_path, exist_ok=True)
    model_config = (
        cfg if isinstance(cfg, BinaryValueConfig) else build_binary_value_config(cfg)
    )
...
def get_model(
    cfg: DictConfig,
    torch_dtype=None,
) -> Union[BinaryValueCriticModel, EnsembleBinaryValueCriticModel]:
    ...
    Returns:
        A :class:`BinaryValueCriticModel` or
        :class:`EnsembleBinaryValueCriticModel` instance.
    """
    del torch_dtype
    ...
    config = build_binary_value_config(cfg, checkpoint_config=checkpoint_config)
    ...
    base_member = BinaryValueCriticModel(config)
    members = clone_ensemble_members(base_member, config.ensemble_size)
    model = EnsembleBinaryValueCriticModel(config, members)
    logger.info(
        "Created EnsembleBinaryValueCriticModel (ARM + ReWiND, ensemble_size=%d)",
...
__all__ = [
    ...
    "EnsembleBinaryValueCriticModel",
    "EnsembleCriticOutput",
    "build_binary_value_config",
    "get_model",
    "load_binary_value_checkpoint_config",
