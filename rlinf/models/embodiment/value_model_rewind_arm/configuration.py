...
"""Binary value model configuration (ARM + ReWiND).

Structurally parallel to :mod:`rlinf.models.embodiment.value_model_evorl.configuration`
— same backbone knobs and state-in-prompt fields — but:
...
fanout in ``modeling_pistar06.Pistar06Model``.
"""
...
VALID_BINARY_VALUE_TARGET_MODES = ("rewind", "positive_only")
...
def normalize_binary_value_target_mode(mode: str) -> str:
    ...
    if mode_norm not in VALID_BINARY_VALUE_TARGET_MODES:
        raise ValueError(
            f"target_mode must be one of {VALID_BINARY_VALUE_TARGET_MODES}, "
            f"got {mode!r}"
        )
    return mode_norm
...
class BinaryValueConfig(PretrainedConfig):
    ...
    model_type = "binary_value_rewind_arm"
    ...
    def __init__(
        ...
        # Interface compat for Pistar06ValueProcessor (state-in-prompt)
        ...
        **kwargs,
        ...
        self.target_mode = normalize_binary_value_target_mode(target_mode)
    ...
    def _validate(self) -> None:
        if not self.vision_repo_id:
            raise ValueError(
                "BinaryValueConfig.vision_repo_id must be a non-empty path or "
                "HF repo id"
            )
        if not self.language_repo_id:
            raise ValueError(
                "BinaryValueConfig.language_repo_id must be a non-empty path or "
                "HF repo id"
            )
